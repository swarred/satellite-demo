...
# ── flightctl-agent ───────────────────────────────────────────────────────────
# Registers the satellite VM with Red Hat Edge Manager on first boot.
# config.yaml is generated by the automation at build time (cluster-specific,
# gitignored) and contains the enrollment endpoint, cert, and CA bundle.
